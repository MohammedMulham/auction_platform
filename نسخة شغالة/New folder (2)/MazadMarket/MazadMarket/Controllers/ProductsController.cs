...
{  
    ...
    public class ProductsController : Controller
    {
        ...
        public ActionResult categoryProductDetails(int? id)
        {
            ...
            {
                ViewBag.MaxPrice = "The Max paied Price :   " + db.ProductsAuctions.Where(t => t.ProductsId == id).Max(t => t.riseAmount);
            }
        ...
        [HttpPost]
        [VerifyExecutionFilter]
        [ValidateAntiForgeryToken]
        public ActionResult categoryProductDetails(int productId,  string remarks, string raise)
        { 
            {
        ...
        public ActionResult categoryProductDetailsAuto(int productId, string valueUP, string limitPrice)
        {
            Products product = (Products)db.Products.Where(a => a.Id == productId).FirstOrDefault();
            Products productnew = (Products)product;
            var person = User.Identity.GetUserId();
            ...
            else
            {
                        ...
                        { 
                            ...
                            if ((db.ProductsAuctions.Where(t => t.ProductsId == productId).Max(t => t.riseAmount) + product.minRaise) > vauleBidding)
                            {
                                         ModelState.AddModelError(string.Empty, "The Offered Price Is Less Than The Min Price");
                            if (list.Count == 0)
                            {
                        ...
                        ProductsAuction productsAuction = new ProductsAuction();
                            ...
                            productsAuction.limitPrice = Convert.ToInt32(stop_condition);
